...
export function VanguardBlogList({ posts, locale }: VanguardBlogListProps) {
  ...
  if (posts.length === 0) {
    return (
      <section className="py-16 md:py-28 bg-background text-foreground px-6 md:px-12 lg:px-24">
        <div className="max-w-2xl">
          <p className="type-meta uppercase tracking-[0.25em] opacity-35 mb-6">
            [ 00 ]
          </p>
          <h2 className="font-display text-[clamp(2.5rem,6vw,5rem)] leading-[0.92] tracking-tight mb-6">
            {t("empty.heading")}
          </h2>
          <p className="type-body opacity-55 mb-10 max-w-md">
            {t("empty.body")}
          </p>
          <Link
            href={`/${locale}#contact`}
            className="inline-flex items-center gap-3 px-6 py-3 border border-foreground type-label-tight hover:bg-foreground hover:text-background transition-colors"
          >
            {t("empty.cta")}
      ...
      <div className="w-full">
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-2 mb-6 md:mb-10 border-b border-foreground/10 pb-5 md:pb-8">
          {categories.map((cat) => (
            <button
              ...
              className={`type-label-tight transition-all pb-1 ${
                activeCategory === cat
        ...
        <div className="space-y-0 min-h-[40vh]">
          {visiblePosts.map((post, idx) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-foreground/[0.03] -translate-x-full group-hover:translate-x-0 transition-transform duration-500 z-0" />
            </Link>
          ))}
          ...
          {filteredPosts.length === 0 && (
            <div className="py-20 text-center">
              <span className="type-label opacity-40">{t("emptyCategory")}</span>
            </div>
          )}
        </div>
        ...
        {filteredPosts.length > 0 && (
          <p className="mt-6 text-center type-meta opacity-30">
            {t("showing", {
              visible: Math.min(visibleCount, filteredPosts.length),
        ...
        <div className="mt-8 md:mt-16 border border-foreground/10 p-5 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 md:gap-6">
          <p className="type-body text-foreground/65">
            {t("cta.text")} <span className="italic">{t("cta.emphasis")}</span>
          </p>
          <Link
            href={`/${locale}#contact`}
